...
def pit_tile(player, round, choices, valid_cols, h_yah, d_yah, roll, swap):
    ...
    while input_err:
        ...
        if use_pit.upper() == "U":
            input_err = False
            undo = place_tile(player, round, h_yah, d_yah, board[player-1][5][5], valid_cols, empty_cells(player), True)
...
def select_tiles(player, round, choices, valid_cols, doubles, h_yah, d_yah, roll, swap):
    if len(choices) == 1 and swap == False:
        ...
        print
        undo = place_tile(player, round, h_yah, d_yah, choices[0], valid_cols, empty_cells(player), False)
        ...
        while sel_input_err:
            ...
            if selection.upper() in choices:
                sel_input_err = False
                undo = place_tile(player, round, h_yah, d_yah, selection.upper(), valid_cols, empty_cells(player), True)
            if doubles and empty_cells(player) > 0:
                (h_yah, d_yah) = print_board(h_yah, d_yah, round, player)
                valid_cols = col_choices(player)
                print "Your roll: " + str(roll)
        ...
        undo = False
    return valid_cols, undo

def place_tile(player, round, h_yah, d_yah, tile, valid_cols, empty_cells, undoable):
    player = player - 1
    if len(valid_cols) == 1:
        if undoable:
            ...
            if placement.upper() == "Z":
                ...
                if placement not in col_strings:
                    ...
                    if board[player][5][placement] == "_":
                        ...
                        pla_input_err = False
                        undo_place = 1
                    print_board(h_yah, d_yah, round, player + 1)
        ...
        while continue_round:
            if empty_cells(player) > 0:
                ...
                if empty_cells(player) > 0:
                    undo = True
                    while undo:
                        valid_cols, undo = select_tiles(player, round, choices, valid_cols, doubles, h_yah, d_yah, roll, swap)
